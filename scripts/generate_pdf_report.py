...
def generate_report(data, output_path):
    ...
    if data.get("competitors"):
        ...
        cell_style = ParagraphStyle(
            "CompCell", parent=body_style, fontSize=8, leading=10, spaceAfter=0)
        header_cell_style = ParagraphStyle(
            "CompHeaderCell", parent=body_style, fontSize=8, leading=10,
            spaceAfter=0, textColor=COLORS["white"], fontName="Helvetica-Bold")
        row_label_style = ParagraphStyle(
            "CompRowLabel", parent=body_style, fontSize=8, leading=10,
            spaceAfter=0, fontName="Helvetica-Bold")

        competitors = data["competitors"][:3]
        header_row = [Paragraph("", header_cell_style)] + [
            Paragraph(c.get("name", f"Competitor {i+1}"), header_cell_style)
        ...
        for row_name in comp_rows:
            row = [Paragraph(row_name, row_label_style)]
            for comp in competitors:
                text = comp.get(row_name.lower().replace(" ", "_"), "—")
                row.append(Paragraph(text, cell_style))
            comp_data.append(row)

        col_count = len(comp_data[0])
        label_width = 80
        data_col_width = (470 - label_width) / (col_count - 1)
    ...
    elements.append(Spacer(1, 0.5 * inch))
    elements.append(Paragraph(
        "Generated by Generated by aigentik.org - Your AI Marketing Audit Assistant",
